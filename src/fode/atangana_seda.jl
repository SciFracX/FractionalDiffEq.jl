function solve(prob::FODEProblem, alg::AtanganaSedaAB; dt = 0.0)
    ...
    tmp1 = zeros(Float64, l)
    tmp2 = zeros(Float64, l)
    f(tmp1, u0, p, t[1])
    result[:, 2] = u0 + dt .* tmp1
    ...
    for n in 3:N
        ...
        for j in 3:n
            ...
            temptemp2 += (temp2 - temp1) * ((n + 1 - j)^order * (n - j + 3 + 2 * order) -
                          (n - j)^order * (n - j + 3 + 3 * order))
            f(temp3, result[:, j], p, t[j])
            temptemp3 += (temp3 - 2 * temp2 + temp1) * ((n - j + 1)^order * (2 * (n - j)^2 +
                           (3 * order + 10) * (n - j) +
                           2 * (order)^2 +
                           9 * order +
                           12) -
        ...
        result[:, n + 1] = u0 +
                           (1 - order) / AB * temptemptemp +
                           ((dt^order) * order / (AB * gamma(order + 1))) * temptemp1 +
                           ((dt^order) * order / (AB * gamma(order + 2))) * temptemp2 +
                           ((dt^order) * order / (2 * AB * gamma(order + 3))) * temptemp3
    end
...
function solve(prob::FODEProblem, ::AtanganaSedaCF; dt = 0.0)
    ...
    for n in 3:N
        ...
        result[:, n + 1] = result[:, n] +
                           (1 - order) / M * (tempn - tempn1) +
                           order *
                           M *
                           dt *
                           (23 / 12 * tempn - 4 / 3 * tempn1 + 5 / 12 * tempn2)
    end
