...
function FOLyapunov(fun, order, t_start, h_norm, t_end, u0, h, out)
    FOLyapunov(fun, order, t_start, h_norm, t_end, u0, h, out, nothing)
end

#FOLyapunov(sys::FODEProblem, h_norm, h, out) = FOLyapunov(sys.f, sys.α, sys.tspan[1], h_norm, sys.tspan[2], sys.u0, h, out)
function FOLyapunov(sys::FODEProblem, h_norm, h, out, p)
    FOLyapunov(sys.f, sys.α, sys.tspan[1], h_norm, sys.tspan[2], sys.u0, h, out, p)
end
function FOLyapunov(sys::FODEProblem, h_norm, h, out)
    FOLyapunov(sys.f, sys.α, sys.tspan[1], h_norm, sys.tspan[2], sys.u0, h, out, nothing)
end
...
function pc(alpha, f_fun, t0, T, y0, h, p)
            ...
            for i_alpha in 1:alpha_length
                ...
                if isempty(find_alpha) == false
                    ...
                    bn_fft[i_alpha, index_fft[1, l]:index_fft[2, l]] = ourfft(
                        METH.bn[i_alpha, 1:coef_end], coef_end)
                    an_fft[i_alpha, index_fft[1, l]:index_fft[2, l]] = ourfft(
                        METH.an[i_alpha, 1:coef_end], coef_end)
                end
            end
        end
        METH.bn_fft = bn_fft
    ...
    y[:, 1] = y0[:, 1]
    fy[:, 1] = f_temp
    (y, fy) = Triangolo(1, r - 1, t, y, fy, zn_pred, zn_corr, N, METH, problem_size,
        alpha_length, m_alpha, m_alpha_factorial, y0, t0, f_fun, p)
    ...
    for qr in 0:Qr
        L = 2^qr
        (y, fy) = DisegnaBlocchi(
            L, ff, r, Nr, nx0 + L * r, ny0, t, y, fy, zn_pred, zn_corr, N, METH,
            problem_size, alpha_length, m_alpha, m_alpha_factorial, y0, t0, f_fun)
...
function DisegnaBlocchi(L, ff, r, Nr, nx0, ny0, t, y, fy, zn_pred, zn_corr, N, METH,
        problem_size, alpha_length, m_alpha, m_alpha_factorial, y0, t0, f_fun)
    nxi = nx0
    nxf = nx0 + L * r - 1
    nyi = ny0
    ...
    while stop == false
        ...
        (zn_pred, zn_corr) = Quadrato(
            nxi, nxf, nyi, nyf, fy, zn_pred, zn_corr, N, METH, problem_size, alpha_length)

        (y, fy) = Triangolo(
            nxi, nxi + r - 1, t, y, fy, zn_pred, zn_corr, N, METH, problem_size,
            alpha_length, m_alpha, m_alpha_factorial, y0, t0, f_fun)
...
function Quadrato(
    ...
    funz_beg = Int64(nyi + 1)
    funz_end = Int64(nyf + 1)
    Nnxf = min(N, nxf)
    ...
    for i in 1:problem_size
        ...
        Z = METH.bn_fft[
            i_alpha, METH.index_fft[1, Int64(i_fft)]:METH.index_fft[2, Int64(i_fft)]] .*
            vett_funz_fft[i, :]
        zzn_pred[i, :] = real.(ourifft(Z, coef_end))
    end
    zzn_pred = zzn_pred[:, Int64(nxf - nyf + 1 - 1):(end - 1)]
    zn_pred[:, Int64(nxi + 1):Int64(Nnxf + 1)] = zn_pred[
        :, Int64(nxi + 1):Int64(Nnxf + 1)] + zzn_pred[:, 1:Int64(Nnxf - nxi + 1)]
    ...
    if METH.mu > 0
        ...
        zn_corr[:, Int64(nxi + 1):Int64(Nnxf + 1)] = zn_corr[
            :, Int64(nxi + 1):Int64(Nnxf + 1)] + zzn_corr[:, 1:Int64(Nnxf - nxi + 1)]
    else
        zn_corr = 0
    end
...
function Triangolo(nxi, nxf, t, y, fy, zn_pred, zn_corr, N, METH, problem_size,
        alpha_length, m_alpha, m_alpha_factorial, y0, t0, f_fun, p)
    for n in Int64(nxi):Int64(min(N, nxf))
        ...
        for j in Int64(j_beg):Int64(n - 1)
        ...
        if METH.mu == 0
            ...
            Phi_n = St +
                    METH.halpha2 .*
                    (METH.a0[1:alpha_length, n + 1] .* fy[:, 1] + zn_corr[:, n + 1] + Phi)
            yn0 = y_pred
            fn0 = f_pred
            stop = 0
            mu_it = 0
