...
function SciMLBase.__init(prob::FDDEProblem, alg::DelayABM; dt = 0.0, kwargs...)
    ...
    if iip
        ...
        @. x[Ndelay + 1] = x0 +
                           dt^order *
                           (f(x[Ndelay + 1], x[1], p, 0) + order * f(x0, x[1], p, 0)) /
                           gamma(order + 2)
    end

    return ABMCache{iip, T}(
        prob, alg, mesh, u0, order, constant_lags, p, x, x0, x1, N, Ndelay, dt, kwargs)
end
    ...
    if iip
        @fastmath @inbounds @simd for n in 1:N
            ...
            @fastmath @inbounds @simd for j in 1:n
                ...
                M1 = @. M1 +
                        ((n - j + 2)^(order + 1) + (n - j)^(order + 1) -
                         2 * (n - j + 1)^(order + 1)) * tmp2
        ...
        @fastmath @inbounds @simd for n in 1:N
            ...
            @fastmath @inbounds @simd for j in 1:n
                @. M1 = M1 +
                        ((n - j + 2)^(order + 1) + (n - j)^(order + 1) -
                         2 * (n - j + 1)^(order + 1)) * prob.f(x[Ndelay + j], x[j], p, 0)
                @. N1 = N1 +
                        ((n - j + 1)^order - (n - j)^order) *
                        prob.f(x[Ndelay + j], x[j], p, 0)
            end
            @. x1[Ndelay + n + 1] = x0 + dt^order * N1 / (gamma(order) * order)
            @. x[Ndelay + n + 1] = x0 +
                                   dt^order *
                                   (prob.f(x[Ndelay + n + 1], x[n + 1], p, 0) + M1) /
                                   gamma(order + 2)
        end
    end
...
function solve(FDDESys::FDDESystem, dt, ::DelayABM)
    ...
    @fastmath @inbounds @simd for n in (2 * Ndelay + 1):(N + Ndelay + 1)
