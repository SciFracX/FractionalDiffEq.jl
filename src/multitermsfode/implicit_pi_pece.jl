...
function MTPECE_quadrato(cache::MTPECECache{T}, nxi, nxf, nyi, nyf) where {T}
    ...
    for i in 1:orders_length
        ...
        cache.zn_pred[:, (nxi + 1):(nxf + 1), i] = cache.zn_pred[
            :, (nxi + 1):(nxf + 1), i] + zzn_pred[:, (nxf - nyf):(end - 1)]
    end
    ...
    if mu > 0
        for i in 1:orders_length
            ...
            cache.zn_corr[:, (nxi + 1):(nxf + 1), i] = cache.zn_corr[
                :, (nxi + 1):(nxf + 1), i] + zzn_corr[:, (nxf - nyf + 1):end]
        end
    else
        cache.zn_corr = 0
    end
...
function MTPECE_triangolo(cache::MTPECECache{T}, nxi, nxf, t0) where {T}
    ...
    for n in nxi:min(N, nxf)
        St = MTPECE_starting_term_multi(mesh[n + 1], t0, problem_size, u0, orders_length,
