...
function PITrap_triangolo(cache::PITrapCache{T}, nxi, nxf, t0) where {T}
    ...
    for n = nxi:min(N, nxf)
        ...
        Gn0 = (1+C)*yn0 .- an[orders_length, 1]./highest_order_parameter*fn0 .- Phi_n
        stop = false; it = 0
        
        while ~stop
            
            JGn0 = (1+C)*(zeros(problem_size, problem_size)+I) .- an[orders_length, 1]/highest_order_parameter*Jfn0
            global yn1 = yn0 - JGn0\Gn0
            global fn1 = prob.f(yn1, p, mesh[n+1])
            Gn1 = (1+C)*yn1 .- an[orders_length, 1]/highest_order_parameter*fn1 .- Phi_n
            it = it + 1
