...
@testset "Test Nonlinear method" begin
    ...
    @test isapprox(test_sol(sol),
        [0.2 0.11749 0.074388 0.0733938 0.117483 0.210073
         -0.1 -0.0590683 -0.018475 0.0192931 0.0534393 0.0844175
         0.1 0.224134 0.282208 0.286636 0.246248 0.168693];
        atol = 1e-3)
end
...
@testset "Test Product Integration explicit method for multi-terms FODE" begin
    ...
    prob = MultiTermsFODEProblem([1, 1 / 16, 4 / 5, 3 / 2, 1 / 25, 6 / 5],
        [3, 2.5, 2, 1, 0.5, 0], rightfun, u0, tspan)
    sol = solve(prob, MTPIEX(), dt = h)

    @test isapprox(test_sol(sol.u[(end - 20):end])',
        [0.2441313200142332
         0.24719587414026023
         0.25016860604787894
         0.25304990316172393
...
@testset "Test Product integration with predictor-corrector method for multi-terms FODE" begin
    ...
    @test isapprox(test_sol(sol.u[(end - 10):end])',
        [0.12517053205360132
         0.128867333370644
         0.13262214977857692
         0.13643478752690427
...
@testset "Test implicit Product integration rectangular type method for multi-terms FODE" begin
    ...
    rightfun(y, p, x) = 172 / 125 * cos(4 / 5 * x)
    prob = MultiTermsFODEProblem([1, 1 / 16, 4 / 5, 3 / 2, 1 / 25, 6 / 5],
        [3, 2.5, 2, 1, 0.5, 0], rightfun, [0, 0, 0, 0, 0, 0], (0, T))
...
@testset "Test product integration PECE method for FODEProblem" begin
    ...
    @test isapprox(test_sol(sol),
        [1.2 1.2061 1.21042 1.21421 1.21767 1.22089 1.22392 1.22678 1.2295 1.2321 1.23459
...
@testset "Test explicit product integration method for FODEProblem" begin
    ...
    @test isapprox(test_sol(sol),
        [1.2 1.20865 1.21458 1.21975 1.22443 1.22874 1.23276 1.23652 1.24006 1.24339 1.24653 1.2495 1.25229 1.25493 1.25575
         2.8 2.77486 2.75941 2.74621 2.73429 2.72326 2.71291 2.70309 2.69373 2.68477 2.67616 2.66786 2.65986 2.65213 2.64964];
        atol = 1e-2)
end
...
@testset "Test NewtonGregory method" begin
    ...
    @test isapprox(test_sol(sol),
        [0.2 0.200531 0.201161 0.201809 0.202453 0.203089
...
@testset "Test Trapezoid method" begin
    ...
    @test isapprox(test_sol(sol),
        [0.2 0.200531 0.201161 0.201808 0.202452 0.203088
         0.03 0.165554 0.265678 0.355635 0.439545 0.519211];
        atol = 1e-4)
end
...
@testset "Test Newton Polynomial" begin
    ...
    @test isapprox(test_sol(sol),
        [-1.0 -1.37074 -1.46124 -1.21771 -0.884241 -0.49327 -0.0897447 0.338635 0.793532 1.2323 1.55187
